...
**Usage counters**:
The running per-account totals — requests, successes, failures, tokens in/out/cache/reasoning — broken down per model for the successes and bucketed per **local calendar day**, summed relay-wide in `status`, shown per account in `accounts`, shown as a 30-day trend in `usage`, and persisted to `usage.json` in the provider's auth directory so they survive restarts and upgrades. Cache writes carry their **1h** share beside the total, because a 1h write bills at 2x base input against 1.25x for 5m and the total alone cannot say what one was worth (ADR-0018); the 5m share is the remainder, never stored twice. **tokens in** is the *uncached* input on every Provider, so `in + cache` is the prompt the upstream saw and no token is counted twice: the Anthropic dialect reports the two cache directions beside `input_tokens`, while the OpenAI dialects report them inside `prompt_tokens` and the relay subtracts them as it records. **tokens out** is every token the model generated, reasoning included: the Anthropic and OpenAI dialects already report it so, and the relay folds a separate reasoning count into it where a vendor keeps the two apart. The field names and inclusion semantics behind both rules per Provider are in `docs/research/cache-usage-fields-by-provider.md`. **A counter counts outcomes, never attempts**: `requests` is incremented by the same call that increments `successes` or `failures`, so `requests == successes + failures` holds cumulatively and per day by construction, and a request still in flight is counted nowhere (ADR-0015). An outcome is booked to the day it arrived on. Counters recorded before per-model attribution or daily bucketing existed stay only in the account totals. Daily buckets are kept for 90 days. Cooldowns and failure streaks are not persisted: a fresh process always retries.
