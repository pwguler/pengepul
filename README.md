...
- `api_mode: anthropic_messages` forces the native wire on a root `base_url`, so the SDK
  appends `/v1/messages` and no extra route is needed.
...
run on the pool. Point it at `http://127.0.0.1:8317` with the local API key.
