...
fn usage_treats_an_out_of_window_history_as_empty() {
    ...
    assert_eq!(
        fact_values(&visible, "tokens"),
        ["none in the last 30 days"],
        "30 flat bars for out-of-window history: {visible}"
    );
    assert_eq!(fact_values(&visible, "peak").len(), 1, "{visible}");
}
...
fn plain_usage_ends_with_the_one_block() {
    ...
    // The daily rows follow the header, and only days inside the window are listed.
    assert!(lines[1].starts_with(&date_back(2)), "{text}");
