...
- **No new endpoint.** The existing `GET /admin/accounts` payload carries
  the breakdown.
- **No unbounded key growth.** Model names come from `upstream_model()`
  after the model is resolved against the config; unknown models are
  rejected before accounting, so the key space is the configured models.
...
- AC-1: `AccountManager::record_success` takes the model name and adds
  ...
  **upstream** model name (what the provider bills), in addition to the
  existing account totals. Both stream and JSON paths pass it.
...
- AC-7: `Style::Plain` `accounts` carries the same information in plain
  ...
  pool footer.
- AC-8: Panel lines stay within 64 visible columns; long model names are
  truncated by the existing panel-row rule, not by a new one.
