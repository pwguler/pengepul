...
- AC-5: `enable` on an account in Reauth clears its Cooldown and says the account needs
  `pengepul login --provider <provider>`. `enable` or `disable` on an account whose credential
  is gone exits non-zero, saying so. An id no Pool holds exits non-zero.
...
- AC-11: A Disabled account keeps its counters and its `lastSuccessAt`, and is listed by
  `accounts`, `status` and `usage` like any other account. The payload carries
